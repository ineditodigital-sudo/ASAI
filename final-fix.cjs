...
// The original distHtml was completely restored by rebuild-master, except earlier scripts modified it.
// Let's restore distribuidores.html from root to guarantee structure is pristine, then remove TRH card perfectly.
...
// B. Apply Swiper custom carousel (just like we did in rebuild-master)
const distImages = [
...
const swiperHtml = \`
<div class="custom-carousel-wrapper" id="swiper-dist" style="margin-top: 40px; margin-bottom: 40px;">
  ...
  <div class="swiper-button-prev swiper-dist-prev"></div>
  <div class="swiper-button-next swiper-dist-next"></div>
</div>
\`;
